...
class CareerGuidanceSystem:
    def __init__(self, google_api_key=None, google_serper_api_key=None):
        """Initialize the career guidance system"""
        self.google_api_key = google_api_key
        self.google_serper_api_key = google_serper_api_key
        
        # Set environment variable for Google API key
        if google_api_key:
            os.environ["GOOGLE_API_KEY"] = google_api_key
        
        # Set environment variable for SerperAPI key
        ...
        if google_api_key:
            ...
            # Initialize search tools if SerperAPI key is provided
            if google_serper_api_key:
                self.search = GoogleSerperAPIWrapper(serper_api_key=google_serper_api_key)
                self.tools = load_tools(["google-serper"], llm=self.llm)
    ...
    def search_with_cache(self, query, cache_key, ttl_hours=24, max_retries=3):
        ...
        if self.search_agent:
            ...
            return "Search unavailable. Please provide a Google Serper API key for web search capabilities."
    ...
    def comprehensive_career_analysis(self, career_name, user_profile=None):
        ...
        try:
            ...
            if self.search_agent and self.google_serper_api_key:
    ...
    def analyze_market_trends(self, career):
        ...
        # Fallback to generic response
        return f"Market analysis for {career} requires web search capabilities. Please provide a Google Serper API key."
    ...
    def create_learning_roadmap(self, career, experience_level="beginner"):
        ...
        # Fallback to generic response
        return f"A personalized learning roadmap for {career} requires web search capabilities. Please provide a Google Serper API key."
    ...
    def get_career_insights(self, career):
        ...
        # Fallback to generic response
        return f"Industry insights for {career} require web search capabilities. Please provide a Google Serper API key."
    
    def chat_with_assistant(self, question, career_data=None):
        """Engage in conversation with a user about career questions"""
        if not self.llm:
            return "Career assistant is not available. Please provide a Google API key."
    ...
    def chat_response(self, user_query, career_data=None, user_profile=None):
        ...
        try:
            ...
            if "<" not in answer:
                ...
                answer = f"<div><p>{formatted_answer}</p></div>"
